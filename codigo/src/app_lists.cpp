...
template<> bool App::extra_commands<Service>(const std::vector<std::string> &s, std::vector<const Service*> &v) {
	if (s[0] == "time" && s.size() == 3) {
		Time t1, t2;
		try { t1.input_date(s[1]); }
		catch (...) { try { t1.input_time(s[1]); } catch (...) { return false; } }
		try { t2.input_date(s[2]); }
		catch (...) { try { t2.input_time(s[2]); } catch (...) { return false; } }
	...
	while (true) {
		...
		if (s.size() >= 1) {
			...
			if (s[0] == "sort") {
				...
				int i; try { i = utils::stoi(s[1]); }
				catch (const std::invalid_argument &e) { error("invalid NUM"); continue; }
